...
public class EmpresaController {
	  ...
	  @RequestMapping("/cadastrarEmpresa") public String form() { return
	  "empresa/form-empresa"; }
	  
	  @RequestMapping(value = "/cadastrarEmpresa", method = RequestMethod.POST)
	  public String form(@Valid Empresa empresa, BindingResult result,
	  RedirectAttributes attributes) {
	  
	  if (result.hasErrors()) { attributes.addFlashAttribute("mensagem",
	  "Verifique os campos..."); return "redirect:/cadastrarEmpresa"; }
	  
	  er.save(empresa); attributes.addFlashAttribute("mensagem",
	  "Empresa cadastrada com sucesso!"); return "redirect:/cadastrarEmpresa"; }
	  
	  // GET que lista as vagas
	  
	  @RequestMapping("/empresas") public ModelAndView listaEmpresas() {
	  ModelAndView mv = new ModelAndView("empresa/lista-empresa");
	  Iterable<Empresa> empresas = er.findAll(); mv.addObject("empresas",
	  empresas); return mv; }
	  
	  @RequestMapping("/deletarEmpresa") public String deletarEmpresa(long empid) {
	  Empresa emp = new Empresa(); emp = er.findByEmpid(empid); er.delete(emp);
	  return "redirect:/empresas"; }
	  
	  @RequestMapping("/editar-empresa") public ModelAndView editarEmpresa(long
	  empid) { Empresa emp = new Empresa(); emp = er.findByEmpid(empid);
	  ModelAndView mv = new ModelAndView("empresa/update-empresa");
	  mv.addObject("emp", emp); return mv; }
	  
	  @RequestMapping(value = "/editar-empresa", method = RequestMethod.POST)
	  public String updateEmpresa(@Valid Empresa emp, BindingResult result,
	  RedirectAttributes attributes) { er.save(emp);
